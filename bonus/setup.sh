...
# argocd CLI setup
echo -n "argocd: " >> password.txt
kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath='{.data.password}' | base64 --decode >> password.txt
...
kubectl wait --for=condition=available deployment/playground -n dev --timeout=600s
